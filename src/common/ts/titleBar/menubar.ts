...
import { MenuItemConstructorOptions, remote, WebContents } from 'electron';
import {
  $,
...
export class Menubar extends Disposable {
	...
	setupMenubar(): void {
	  const topLevelMenus = this.options?.menu?.items as unknown as MenuItemConstructorOptions[];
	...
	private onClick(menuIndex: number) {
	  ...
	  if (item.click) {
	    item.click(item as unknown as Electron.MenuItem, remote.getCurrentWindow(), electronEvent);
	  }
	}
	...
	private updateLabels(titleElement: HTMLElement, buttonElement: HTMLElement, label: string): void {
	  ...
	  titleElement.innerHTML = this.options?.enableMnemonics ?
	    escape(label).replace(MENU_ESCAPED_MNEMONIC_REGEX, '<mnemonic aria-hidden="true">$1</mnemonic>') :
	    cleanMenuLabel;
	  ...
	  if (this.menuItems) {
	    this.menuItems.forEach(menuBarMenu => {
	      if (menuBarMenu.titleElement.children.length) {
	        ...
	        if (child) {
	          child.style.textDecoration = visible ? 'underline' : '';
	        }
	      }
	    });
	  }
	...
	private showMenu(menuIndex: number, selectFirst = true): void {
	  ...
	  let menuOptions: IMenuOptions = {
	    ...
	    ariaLabel: customMenu.buttonElement.attributes.getNamedItem('aria-label')?.value
	  };
