...
export class Menu extends Disposable {
	...
	createMenu(items: IMenuItem[]) {
	  items.forEach((menuItem: IMenuItem) => {
	    ...
	    if (menuItem.type === 'separator') {
	      ...
	      const submenuItems = (menuItem.submenu as Electron.Menu).items as unknown as IMenuItem[];
	...
	style(style: IMenuStyle) {
	  ...
	  container.style.backgroundColor = style.backgroundColor ? style.backgroundColor.toString() : '';
...
class Separator extends MenuItem {
	...
	style(style: IMenuStyle) {
	  this.separatorElement.style.borderBottomColor = style.separatorColor ? `${style.separatorColor}` : '';
	}
}
