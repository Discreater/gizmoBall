  ...
  public crashHandle(crashable:ICollisible):void {
    ...
    if (crashable instanceof Ball) {
      ...
      if (crashLine !== null) {
        ...
        if (crashLine.p1.equals(edges[0].p1) || crashLine.p1.equals(edges[3].p1)) {
          // 侧边
          // console.log("side!");
        ...
        if (Vector2D.difference(crashable.center, this.center).radius < 5) {
          if (this.collider.entryAxis1.dot(v) < 0) {
            ...
            massPoint.setVelocity(v.clone().directionRotate(this.collider.entryAxis2));
          } else if (this.collider.entryAxis2.dot(v) < 0) {
            // 从entryAxis2进入
            crashable.translate(this.position);
            massPoint.setVelocity(v.clone().directionRotate(this.collider.entryAxis1));
          }
        }
