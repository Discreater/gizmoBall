<template>
  <img @dragstart="onDragstart($event)" @dragend="onDragend()" :class="cssClass"
    ref="img" :src="item.imgURL" :width="`${width}px`" :height="`${height}px`" :draggable="imageDraggable"
    :style="`position: absolute;top:${item.position.y}px;left:${item.position.x}px;z-index:${zindex};transform: rotate(${rotation}deg)`"/>
</template>
